...
public class GE_GE_PRP_EOL {
   ...
   public static double    server_utilization;
    ...
    public static int num_delayed_PU,num_delayed_CU,lost_PU,lost_CU, SU_preempted;
   ...
   public static    double[] time_arrival_CU=new double[Q_SIZE+1];
    public static    double[] CU_remain_time=new double[Q_SIZE+2];
     public static    double[] CU_Q_position=new double[Q_SIZE+2];
...
//------------------------------------------the main program.---------------
      ...
      {
        //sim_out2= new PrintWriter(new FileWriter("CRoutput.xls"));
        sim_out2.println("\n*********PRP EOL***********");
       sim_out2.println("M_Utilization\tM_Q len_PU\tM_Q len_SU");
...

   {
      ...
      confidence_calculation();
        KK+=1;
         ...
         System.out.println("The output of PRP EOL has been written to the file");
   // sim_out2.close();
      }
      ...
      static void premptServer()
        {
          ...
          if(num_in_q_CU < Q_SIZE-1 && SU_preempted< Q_SIZE-1)
          {
              num_in_q_CU++;
              SU_preempted++;
              CU_remain_time[SU_preempted] =time_next_event[2]-sim_time;
               try {

                    CU_Q_position[SU_preempted] = num_in_q_CU;
      ...
      static void servePremptedCU()
        {
          ...
          try {
                for(int i = 1; i<= SU_preempted; i++)

                        CU_remain_time[i]=CU_remain_time[i+1];
...

      {
         ...
         try {
       ...
       else if(num_in_q_CU >0)
           {
            ...
            if(SU_preempted <0 && CU_Q_position[1]==1)
                servePremptedCU();
            else {ST=GE(a2,mu);
            //num_customer++;
            }
         ...
         try {
            ...
            for(int i=1;i<= SU_preempted;i++)
                ...
                CU_Q_position[i]=CU_Q_position[i]-1;
            }
             catch(ArrayIndexOutOfBoundsException e) { e.printStackTrace();}
             num_in_q_CU--;
           }
